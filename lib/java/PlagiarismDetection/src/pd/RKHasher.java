...
public class RKHasher {
    ...
    private Map<Integer, ArrayList<Integer>> precomputedHashesToIndices = new HashMap<>();
    ...
    */
    public void precomputeHashesForNGramList(NGramList ngList) {
        ...
        for (int i = windowStart; i <= windowEnd; i++) {
            int hash = ngList.get(i).nGramHash().intValue() % base;
            currentHash += hash * Math.pow(base, windowSize - i);
        }
        precomputedHashes.add(currentHash);
        addToPrecomputedHashesToIndices(currentHash, ngList.get(windowStart));
    ...
    public boolean precomputedHashesContains(int hash) {
        return precomputedHashesToIndices.containsKey(hash);
    }

    public ArrayList<Integer> getMatchingStartPositions(int hash) {
        return precomputedHashesToIndices.get(hash);
    }

    private void addToPrecomputedHashesToIndices(int hash, NGram ng) {
        if (!precomputedHashesToIndices.containsKey(hash)) {
            precomputedHashesToIndices.put(hash, new ArrayList<>());
            precomputedHashesToIndices.get(hash).add(ng.codeStartIndex());
        } else {
            precomputedHashesToIndices.get(hash).add(ng.codeStartIndex());
        }
    }
